...
static void
xfpm_idle_x11_xsync_alarm_set (XfpmIdleX11 *idle,
                               Alarm *alarm,
...
static Alarm *
xfpm_idle_x11_alarm_find_id (XfpmIdleX11 *idle,
                             XfpmAlarmId id)
{
  for (guint i = 0; i < idle->array->len; i++)
  {
    Alarm *alarm = g_ptr_array_index (idle->array, i);
    if (alarm->id == id)
      return alarm;
  }
  return NULL;
...
static void
xfpm_idle_x11_set_reset_alarm (XfpmIdleX11 *idle,
                               XSyncAlarmNotifyEvent *alarm_event)
{
  Alarm *alarm = xfpm_idle_x11_alarm_find_id (idle, 0);
  if (!idle->reset_set)
  {
    int overflow;
    XSyncValue add;
    ...
    xfpm_idle_x11_xsync_alarm_set (idle, alarm, ALARM_TYPE_NEGATIVE);
...
static Alarm *
xfpm_idle_x11_alarm_find_event (XfpmIdleX11 *idle,
                                XSyncAlarmNotifyEvent *alarm_event)
{
  for (guint i = 0; i < idle->array->len; i++)
  {
...
xfpm_idle_x11_event_filter_cb (GdkXEvent *gdkxevent,
                               GdkEvent *event,
                               gpointer data)
{
  Alarm *alarm;
  ...
  alarm = xfpm_idle_x11_alarm_find_event (idle, alarm_event);
  if (alarm == NULL)
    return GDK_FILTER_CONTINUE;
  ...
  xfpm_idle_x11_set_reset_alarm (idle, alarm_event);
out:
  /* don't propagate */
...
static Alarm *
xfpm_idle_x11_alarm_new (XfpmIdleX11 *idle,
                         XfpmAlarmId id)
{
  /* create a new alarm */
...
static void
xfpm_idle_x11_alarm_free (gpointer data)
{
  Alarm *alarm = data;
  if (alarm->xalarm)
...
xfpm_idle_x11_init (XfpmIdleX11 *idle)
{
  ...
  idle->array = g_ptr_array_new_with_free_func (xfpm_idle_x11_alarm_free);
  ...
  gdk_window_add_filter (NULL, xfpm_idle_x11_event_filter_cb, idle);

  /* create a reset alarm */
  alarm = xfpm_idle_x11_alarm_new (idle, 0);
  g_ptr_array_add (idle->array, alarm);
}
...
xfpm_idle_x11_finalize (GObject *object)
{
  ...
  g_ptr_array_free (idle->array, TRUE);
...
xfpm_idle_x11_alarm_reset_all (XfpmIdle *_idle)
{
  ...
  for (guint i = 1; i < idle->array->len; i++)
  {
    alarm = g_ptr_array_index (idle->array, i);
    xfpm_idle_x11_xsync_alarm_set (idle, alarm, ALARM_TYPE_POSITIVE);
  }

  /* set the reset alarm to be disabled */
  alarm = g_ptr_array_index (idle->array, 0);
  xfpm_idle_x11_xsync_alarm_set (idle, alarm, ALARM_TYPE_DISABLED);
                         ...
                         guint timeout)
{
  ...
  Alarm *alarm = xfpm_idle_x11_alarm_find_id (idle, id);
  if (alarm == NULL)
  {
    /* create a new alarm */
    alarm = xfpm_idle_x11_alarm_new (idle, id);
                            ...
                            XfpmAlarmId id)
{
  ...
  g_ptr_array_remove (idle->array, alarm);
}
